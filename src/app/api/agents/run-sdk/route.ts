...
/**
 * E92 · CC#3 · 2026-09-17 · RECUPERAR LA RESPUESTA YA PAGADA (arreglo A de E86).
 *
 * Si la conexión con el corredor se cae mientras se lee la respuesta, el empleado **ya
 * terminó y ya se cobró**: su respuesta queda en `agent_invocations`. Antes se tiraba y a
 * veces se reintentaba (la bolita E83 pagó ~US$ 0,98 de más por dos cortes).
 *
 * Tope de espera · **120 s** · POR QUÉ: medido el 17-sep, la fila apareció **20 s** después
 * de un corte y **58 s** después del otro (E91 corrigió el «18 s» de E86). 120 s es 2× el
 * peor caso medido. ⚠️ Un corte temprano sobre un empleado MUY lento (un plan de ~9 min)
 * excede el tope a propósito: pasado el tope **es «no sé»**, nunca un valor inventado
 * (el arreglo C de CC#1 ya convierte ese «no sé» en parada ruidosa aguas abajo).
 *
...
export function respuestaDesdeLaFila(fila: FilaInvocacion): AgentRunResultProxy {
  const m = (fila.metadata ?? {}) as Record<string, unknown>
...
async function buscarFilaDelEmpleado(
  agente: string,
  paso: string | null,
  executionId: string | null,
  intentoEmpezoEnMs: number,
  ...
  let q = supabase
    ...
    .gte('started_at', new Date(intentoEmpezoEnMs - 60_000).toISOString())
...
export async function recuperarRespuestaPagada(
...
): Promise<AgentRunResultProxy | null> {
  ...
  const empezo = ahora()
  const comun =
    `agent=${ctx.agent} · step=${ctx.step ?? '-'} · exec=${ctx.executionId ?? '-'} · tope=${Math.round(topeMs / 1000)}s`
  ...
  for (let vuelta = 1; ; vuelta++) {
    ...
    if (fila) {
      ...
      if (entera) {
        ...
        console.warn(
          `[run-sdk] respuesta_recuperada · la conexión se cayó pero la respuesta ya estaba pagada y entera · ` +
      ...
      if (motivo && motivo.startsWith('recortada')) break
      if (motivo && motivo.startsWith('la fila declara')) break
      if (motivo && motivo.startsWith('la fila no declara')) break
    }
    if (ahora() - empezo + sondeoMs >= topeMs) break
    await esperar(sondeoMs)
  }
  console.warn(
    `[run-sdk] respuesta_no_recuperada · sigue siendo «no sé» (nunca un valor inventado) · ` +
  ...
  try {
    ...
    for (let attempt = 1; ; attempt++) {
      ...
      try {
        ...
        // E92 · la conexión se cayó, pero el empleado YA terminó y YA se cobró · su respuesta
        // está en `agent_invocations`. Se espera la fila y se devuelve la respuesta REAL.
        // Si no se puede probar que está entera, o nunca llega ⇒ sube el mismo error de
        // siempre y aguas abajo es «no sé» (arreglo C · CC#1). Camino normal intacto.
        const recuperada = await recuperarRespuestaPagada(
          {
            agent: agentName,
            step: body.step_name ?? null,
            executionId: wfAttr.workflow_execution_id,
            desdeIso: new Date(attemptStartedAt - 60_000).toISOString(),
          },
          { buscarFila: () => buscarFilaDelEmpleado(agentName, body.step_name ?? null, wfAttr.workflow_execution_id, attemptStartedAt) },
        )
        if (!recuperada) throw errLectura
        railwayText = JSON.stringify(recuperada)
      }
