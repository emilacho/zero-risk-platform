...
import { buscarFilaDelEmpleado, recuperarRespuestaPagada as recuperarRespuestaPagadaImpl } from '@/lib/respuesta-pagada'
import {
...
/**
 * E92/E96 · la recuperación de la respuesta ya pagada vive en `@/lib/respuesta-pagada`
 * (la usan `run-sdk` y el rescate del manual de marca · arreglo D). Se re-exporta acá porque
 * las pruebas de E92 la nombran desde esta ruta.
 */
export {
  RECUPERACION_TOPE_MS,
  RECUPERACION_SONDEO_MS,
  filaEstaEntera,
  ...
  try {
    ...
    for (let attempt = 1; ; attempt++) {
      ...
      try {
        ...
        const recuperada = await recuperarRespuestaPagadaImpl(
          {
            agent: agentName,
            step: body.step_name ?? null,
            executionId: wfAttr.workflow_execution_id,
          ...
          {
            buscarFila: () =>
              buscarFilaDelEmpleado(agentName, body.step_name ?? null, wfAttr.workflow_execution_id, attemptStartedAt),
          },
        )
