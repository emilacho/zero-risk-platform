...
const visual = {
  ...
  ficha: 'no_aplica',
  error: null,
  nota: 'sólo lo declarado en el HTML/CSS del sitio · se guarda en la ficha únicamente si estaba vacía · vacío = no se encontró, no se inventa',
}
...
try {
  ...
  const css = []
  for (const h of hojasDeEstilo(html, url).slice(0, 3)) {
    try {
      ...
      visual.hojas_leidas.push({ url: h, bytes: t.length })
    } catch (e) {
