...
async function checkEndpointAuth() {
  ...
  for (const file of routeFiles) {
    ...
    const hasAuth =
      /requireInternalKey|checkInternalKey|checkInternalOrAdmin|requireAdmin|verifyTallyWebhook|verifyMetaSignature|verifyWebhookSignature|ADMIN_SECRET|pipeline_secret|PIPELINE_CALLBACK_SECRET|CRON_SECRET|SUPABASE_SERVICE_ROLE_KEY|verifySignature|handleReadStub|handleStubPost|handleStub\b|buildDeprecatedResponse/i.test(
        content,
      )
    if (!hasAuth) {
      addError(
