-- ============================================================================
-- RLS Test: public.content_packages
...
-- Run: psql -f supabase/tests/rls/content_packages.sql · OR Supabase SQL Editor
-- Pre-flight: migration 202604300001_rls_multi_tenant_content_packages.sql applied
...
BEGIN
  SELECT COUNT(*) INTO visible_count FROM public.content_packages;
  SELECT COUNT(*) INTO cross_tenant_visible FROM public.content_packages
...
BEGIN
  SELECT COUNT(*) INTO client_a_visible FROM public.content_packages
    WHERE client_id = ('00000000-0000-0000-0000-aaaaaaaaaaaa')::uuid;
...
DECLARE
  total_rows integer;
BEGIN
  SELECT COUNT(*) INTO total_rows FROM public.content_packages;
...
DECLARE
  anon_count integer;
BEGIN
  SELECT COUNT(*) INTO anon_count FROM public.content_packages;
