...
-- Run: psql -f supabase/tests/rls/rfm_segments.sql · OR Supabase SQL Editor
-- Pre-flight: migration 202604300001_rls_multi_tenant_rfm_segments.sql applied
...
BEGIN
  SELECT COUNT(*) INTO visible_count FROM public.rfm_segments;
  SELECT COUNT(*) INTO cross_tenant_visible FROM public.rfm_segments
...
BEGIN
  SELECT COUNT(*) INTO client_a_visible FROM public.rfm_segments
    WHERE client_id = ('sandbox-client-a');
...
DECLARE
  total_rows integer;
BEGIN
  SELECT COUNT(*) INTO total_rows FROM public.rfm_segments;
...
DECLARE
  anon_count integer;
BEGIN
  SELECT COUNT(*) INTO anon_count FROM public.rfm_segments;
