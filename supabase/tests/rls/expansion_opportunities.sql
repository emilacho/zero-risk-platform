-- ============================================================================
-- RLS Test: public.expansion_opportunities
...
-- Run: psql -f supabase/tests/rls/expansion_opportunities.sql · OR Supabase SQL Editor
-- Pre-flight: migration 202604300001_rls_multi_tenant_expansion_opportunities.sql applied
...
BEGIN
  SELECT COUNT(*) INTO visible_count FROM public.expansion_opportunities;
  SELECT COUNT(*) INTO cross_tenant_visible FROM public.expansion_opportunities
...
BEGIN
  SELECT COUNT(*) INTO client_a_visible FROM public.expansion_opportunities
    WHERE client_id = ('sandbox-client-a');
...
DECLARE
  total_rows integer;
BEGIN
  SELECT COUNT(*) INTO total_rows FROM public.expansion_opportunities;
...
DECLARE
  anon_count integer;
BEGIN
  SELECT COUNT(*) INTO anon_count FROM public.expansion_opportunities;
