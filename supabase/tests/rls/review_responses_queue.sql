-- ============================================================================
-- RLS Test: public.review_responses_queue
...
-- Run: psql -f supabase/tests/rls/review_responses_queue.sql · OR Supabase SQL Editor
-- Pre-flight: migration 202604300001_rls_multi_tenant_review_responses_queue.sql applied
...
BEGIN
  SELECT COUNT(*) INTO visible_count FROM public.review_responses_queue;
  SELECT COUNT(*) INTO cross_tenant_visible FROM public.review_responses_queue
...
BEGIN
  SELECT COUNT(*) INTO client_a_visible FROM public.review_responses_queue
    WHERE client_id = ('sandbox-client-a');
...
DECLARE
  total_rows integer;
BEGIN
  SELECT COUNT(*) INTO total_rows FROM public.review_responses_queue;
...
DECLARE
  anon_count integer;
BEGIN
  SELECT COUNT(*) INTO anon_count FROM public.review_responses_queue;
