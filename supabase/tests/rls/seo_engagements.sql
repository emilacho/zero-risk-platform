-- ============================================================================
-- RLS Test: public.seo_engagements
...
-- Run: psql -f supabase/tests/rls/seo_engagements.sql · OR Supabase SQL Editor
-- Pre-flight: migration 202604300001_rls_multi_tenant_seo_engagements.sql applied
...
BEGIN
  SELECT COUNT(*) INTO visible_count FROM public.seo_engagements;
  SELECT COUNT(*) INTO cross_tenant_visible FROM public.seo_engagements
...
BEGIN
  SELECT COUNT(*) INTO client_a_visible FROM public.seo_engagements
    WHERE client_id = ('00000000-0000-0000-0000-aaaaaaaaaaaa')::uuid;
...
DECLARE
  total_rows integer;
BEGIN
  SELECT COUNT(*) INTO total_rows FROM public.seo_engagements;
...
DECLARE
  anon_count integer;
BEGIN
  SELECT COUNT(*) INTO anon_count FROM public.seo_engagements;
