-- ============================================================================
-- RLS Test: public.rank_tracking_daily
...
-- Run: psql -f supabase/tests/rls/rank_tracking_daily.sql · OR Supabase SQL Editor
-- Pre-flight: migration 202604300001_rls_multi_tenant_rank_tracking_daily.sql applied
...
BEGIN
  SELECT COUNT(*) INTO visible_count FROM public.rank_tracking_daily;
  SELECT COUNT(*) INTO cross_tenant_visible FROM public.rank_tracking_daily
...
BEGIN
  SELECT COUNT(*) INTO client_a_visible FROM public.rank_tracking_daily
    WHERE client_id = ('00000000-0000-0000-0000-aaaaaaaaaaaa')::uuid;
...
DECLARE
  total_rows integer;
BEGIN
  SELECT COUNT(*) INTO total_rows FROM public.rank_tracking_daily;
...
DECLARE
  anon_count integer;
BEGIN
  SELECT COUNT(*) INTO anon_count FROM public.rank_tracking_daily;
