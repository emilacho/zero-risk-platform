-- ============================================================================
-- RLS Test: public.social_metrics
...
-- Run: psql -f supabase/tests/rls/social_metrics.sql · OR Supabase SQL Editor
-- Pre-flight: migration 202604300001_rls_multi_tenant_social_metrics.sql applied
...
BEGIN
  SELECT COUNT(*) INTO visible_count FROM public.social_metrics;
  SELECT COUNT(*) INTO cross_tenant_visible FROM public.social_metrics
...
BEGIN
  SELECT COUNT(*) INTO client_a_visible FROM public.social_metrics
    WHERE client_id = ('00000000-0000-0000-0000-aaaaaaaaaaaa')::uuid;
...
DECLARE
  total_rows integer;
BEGIN
  SELECT COUNT(*) INTO total_rows FROM public.social_metrics;
...
DECLARE
  anon_count integer;
BEGIN
  SELECT COUNT(*) INTO anon_count FROM public.social_metrics;
