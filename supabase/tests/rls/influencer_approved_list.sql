-- ============================================================================
-- RLS Test: public.influencer_approved_list
...
-- Run: psql -f supabase/tests/rls/influencer_approved_list.sql · OR Supabase SQL Editor
-- Pre-flight: migration 202604300001_rls_multi_tenant_influencer_approved_list.sql applied
...
SET LOCAL request.jwt.claims = '{"client_id":"sandbox-client-a"}';
DO $$
DECLARE
  visible_count integer;
  cross_tenant_visible integer;
BEGIN
  SELECT COUNT(*) INTO visible_count FROM public.influencer_approved_list;
  SELECT COUNT(*) INTO cross_tenant_visible FROM public.influencer_approved_list
    WHERE client_id <> ('sandbox-client-a');
...
SET LOCAL request.jwt.claims = '{"client_id":"sandbox-client-b"}';
DO $$
DECLARE
  client_a_visible integer;
BEGIN
  SELECT COUNT(*) INTO client_a_visible FROM public.influencer_approved_list
    WHERE client_id = ('sandbox-client-a');
...
DECLARE
  total_rows integer;
BEGIN
  SELECT COUNT(*) INTO total_rows FROM public.influencer_approved_list;
...
DECLARE
  anon_count integer;
BEGIN
  SELECT COUNT(*) INTO anon_count FROM public.influencer_approved_list;
