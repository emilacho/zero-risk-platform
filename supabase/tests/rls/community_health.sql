-- ============================================================================
-- RLS Test: public.community_health
...
-- Run: psql -f supabase/tests/rls/community_health.sql · OR Supabase SQL Editor
-- Pre-flight: migration 202604300001_rls_multi_tenant_community_health.sql applied
...
BEGIN
  SELECT COUNT(*) INTO visible_count FROM public.community_health;
  SELECT COUNT(*) INTO cross_tenant_visible FROM public.community_health
...
BEGIN
  SELECT COUNT(*) INTO client_a_visible FROM public.community_health
    WHERE client_id = ('sandbox-client-a');
...
DECLARE
  total_rows integer;
BEGIN
  SELECT COUNT(*) INTO total_rows FROM public.community_health;
...
DECLARE
  anon_count integer;
BEGIN
  SELECT COUNT(*) INTO anon_count FROM public.community_health;
