-- ============================================================================
-- RLS Test: public.incrementality_tests
...
-- Run: psql -f supabase/tests/rls/incrementality_tests.sql · OR Supabase SQL Editor
-- Pre-flight: migration 202604300001_rls_multi_tenant_incrementality_tests.sql applied
...
BEGIN
  SELECT COUNT(*) INTO visible_count FROM public.incrementality_tests;
  SELECT COUNT(*) INTO cross_tenant_visible FROM public.incrementality_tests
...
BEGIN
  SELECT COUNT(*) INTO client_a_visible FROM public.incrementality_tests
    WHERE client_id = ('00000000-0000-0000-0000-aaaaaaaaaaaa')::uuid;
...
DECLARE
  total_rows integer;
BEGIN
  SELECT COUNT(*) INTO total_rows FROM public.incrementality_tests;
...
DECLARE
  anon_count integer;
BEGIN
  SELECT COUNT(*) INTO anon_count FROM public.incrementality_tests;
