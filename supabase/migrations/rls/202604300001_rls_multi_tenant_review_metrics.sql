-- ============================================================================
-- Migration: 202604300001_rls_multi_tenant_review_metrics
-- Purpose: Replace permissive 'authenticated_read' RLS policy on public.review_metrics
...
DROP POLICY IF EXISTS "authenticated_read" ON public.review_metrics;
DROP POLICY IF EXISTS "authenticated_read_review_metrics" ON public.review_metrics;

-- 3. Re-assert service_role bypass (Mission Control + n8n + backend admin)
DROP POLICY IF EXISTS "service_role_all" ON public.review_metrics;
CREATE POLICY "service_role_all" ON public.review_metrics
  FOR ALL
  TO service_role
  USING (true)
  WITH CHECK (true);
...
DROP POLICY IF EXISTS "client_isolation_review_metrics_select" ON public.review_metrics;
CREATE POLICY "client_isolation_review_metrics_select" ON public.review_metrics
  FOR SELECT
  TO authenticated
  USING (
...
DROP POLICY IF EXISTS "client_isolation_review_metrics_insert" ON public.review_metrics;
CREATE POLICY "client_isolation_review_metrics_insert" ON public.review_metrics
  FOR INSERT
  TO authenticated
  WITH CHECK (
    client_id = (auth.jwt() ->> 'client_id')::uuid
...
DROP POLICY IF EXISTS "client_isolation_review_metrics_update" ON public.review_metrics;
CREATE POLICY "client_isolation_review_metrics_update" ON public.review_metrics
  FOR UPDATE
  TO authenticated
  USING (
    client_id = (auth.jwt() ->> 'client_id')::uuid
...
DROP POLICY IF EXISTS "client_isolation_review_metrics_delete" ON public.review_metrics;
CREATE POLICY "client_isolation_review_metrics_delete" ON public.review_metrics
  FOR DELETE
  TO authenticated
  USING (
    client_id = (auth.jwt() ->> 'client_id')::uuid
...
COMMENT ON TABLE public.review_metrics IS
...
-- DROP POLICY IF EXISTS "client_isolation_review_metrics_update" ON public.review_metrics;
-- DROP POLICY IF EXISTS "client_isolation_review_metrics_delete" ON public.review_metrics;
-- CREATE POLICY "authenticated_read" ON public.review_metrics
