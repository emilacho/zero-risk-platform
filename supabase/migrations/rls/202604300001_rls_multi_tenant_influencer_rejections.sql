-- ============================================================================
-- Migration: 202604300001_rls_multi_tenant_influencer_rejections
-- Purpose: Replace permissive 'authenticated_read' RLS policy on public.influencer_rejections
...
ALTER TABLE public.influencer_rejections ENABLE ROW LEVEL SECURITY;

-- 2. Drop the legacy permissive policies (multiple legacy names possible)
DROP POLICY IF EXISTS "authenticated_read" ON public.influencer_rejections;
DROP POLICY IF EXISTS "authenticated_read_influencer_rejections" ON public.influencer_rejections;

-- 3. Re-assert service_role bypass (Mission Control + n8n + backend admin)
DROP POLICY IF EXISTS "service_role_all" ON public.influencer_rejections;
CREATE POLICY "service_role_all" ON public.influencer_rejections
  FOR ALL
  TO service_role
  USING (true)
  WITH CHECK (true);
...
DROP POLICY IF EXISTS "client_isolation_influencer_rejections_select" ON public.influencer_rejections;
CREATE POLICY "client_isolation_influencer_rejections_select" ON public.influencer_rejections
  FOR SELECT
  TO authenticated
  USING (
...
DROP POLICY IF EXISTS "client_isolation_influencer_rejections_insert" ON public.influencer_rejections;
CREATE POLICY "client_isolation_influencer_rejections_insert" ON public.influencer_rejections
  FOR INSERT
  TO authenticated
  WITH CHECK (
    client_id = (auth.jwt() ->> 'client_id')
...
DROP POLICY IF EXISTS "client_isolation_influencer_rejections_update" ON public.influencer_rejections;
CREATE POLICY "client_isolation_influencer_rejections_update" ON public.influencer_rejections
  FOR UPDATE
  TO authenticated
  USING (
    client_id = (auth.jwt() ->> 'client_id')
...
DROP POLICY IF EXISTS "client_isolation_influencer_rejections_delete" ON public.influencer_rejections;
CREATE POLICY "client_isolation_influencer_rejections_delete" ON public.influencer_rejections
  FOR DELETE
  TO authenticated
  USING (
    client_id = (auth.jwt() ->> 'client_id')
...
COMMENT ON TABLE public.influencer_rejections IS
...
-- DROP POLICY IF EXISTS "client_isolation_influencer_rejections_update" ON public.influencer_rejections;
-- DROP POLICY IF EXISTS "client_isolation_influencer_rejections_delete" ON public.influencer_rejections;
-- CREATE POLICY "authenticated_read" ON public.influencer_rejections
