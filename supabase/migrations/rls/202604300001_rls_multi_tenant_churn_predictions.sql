-- ============================================================================
-- Migration: 202604300001_rls_multi_tenant_churn_predictions
-- Purpose: Replace permissive 'authenticated_read' RLS policy on public.churn_predictions
...
DROP POLICY IF EXISTS "authenticated_read" ON public.churn_predictions;
DROP POLICY IF EXISTS "authenticated_read_churn_predictions" ON public.churn_predictions;

-- 3. Re-assert service_role bypass (Mission Control + n8n + backend admin)
DROP POLICY IF EXISTS "service_role_all" ON public.churn_predictions;
CREATE POLICY "service_role_all" ON public.churn_predictions
  FOR ALL
  TO service_role
  USING (true)
  WITH CHECK (true);
...
DROP POLICY IF EXISTS "client_isolation_churn_predictions_select" ON public.churn_predictions;
CREATE POLICY "client_isolation_churn_predictions_select" ON public.churn_predictions
  FOR SELECT
  TO authenticated
  USING (
...
DROP POLICY IF EXISTS "client_isolation_churn_predictions_insert" ON public.churn_predictions;
CREATE POLICY "client_isolation_churn_predictions_insert" ON public.churn_predictions
  FOR INSERT
  TO authenticated
  WITH CHECK (
    client_id = (auth.jwt() ->> 'client_id')
...
DROP POLICY IF EXISTS "client_isolation_churn_predictions_update" ON public.churn_predictions;
CREATE POLICY "client_isolation_churn_predictions_update" ON public.churn_predictions
  FOR UPDATE
  TO authenticated
  USING (
    client_id = (auth.jwt() ->> 'client_id')
...
DROP POLICY IF EXISTS "client_isolation_churn_predictions_delete" ON public.churn_predictions;
CREATE POLICY "client_isolation_churn_predictions_delete" ON public.churn_predictions
  FOR DELETE
  TO authenticated
  USING (
    client_id = (auth.jwt() ->> 'client_id')
...
-- DROP POLICY IF EXISTS "client_isolation_churn_predictions_update" ON public.churn_predictions;
-- DROP POLICY IF EXISTS "client_isolation_churn_predictions_delete" ON public.churn_predictions;
-- CREATE POLICY "authenticated_read" ON public.churn_predictions
