-- ============================================================================
-- Migration: 202604300001_rls_multi_tenant_email_sequences
-- Purpose: Replace permissive 'authenticated_read' RLS policy on public.email_sequences
...
DROP POLICY IF EXISTS "authenticated_read" ON public.email_sequences;
DROP POLICY IF EXISTS "authenticated_read_email_sequences" ON public.email_sequences;

-- 3. Re-assert service_role bypass (Mission Control + n8n + backend admin)
DROP POLICY IF EXISTS "service_role_all" ON public.email_sequences;
...
DROP POLICY IF EXISTS "client_isolation_email_sequences_select" ON public.email_sequences;
CREATE POLICY "client_isolation_email_sequences_select" ON public.email_sequences
  FOR SELECT
  TO authenticated
  USING (
...
DROP POLICY IF EXISTS "client_isolation_email_sequences_insert" ON public.email_sequences;
CREATE POLICY "client_isolation_email_sequences_insert" ON public.email_sequences
  FOR INSERT
  TO authenticated
  WITH CHECK (
    client_id = (auth.jwt() ->> 'client_id')
...
DROP POLICY IF EXISTS "client_isolation_email_sequences_update" ON public.email_sequences;
CREATE POLICY "client_isolation_email_sequences_update" ON public.email_sequences
  FOR UPDATE
  TO authenticated
  USING (
    client_id = (auth.jwt() ->> 'client_id')
...
DROP POLICY IF EXISTS "client_isolation_email_sequences_delete" ON public.email_sequences;
CREATE POLICY "client_isolation_email_sequences_delete" ON public.email_sequences
  FOR DELETE
  TO authenticated
  USING (
    client_id = (auth.jwt() ->> 'client_id')
...
-- DROP POLICY IF EXISTS "client_isolation_email_sequences_update" ON public.email_sequences;
-- DROP POLICY IF EXISTS "client_isolation_email_sequences_delete" ON public.email_sequences;
-- CREATE POLICY "authenticated_read" ON public.email_sequences
