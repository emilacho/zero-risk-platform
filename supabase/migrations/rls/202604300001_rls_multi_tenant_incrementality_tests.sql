-- ============================================================================
-- Migration: 202604300001_rls_multi_tenant_incrementality_tests
-- Purpose: Replace permissive 'authenticated_read' RLS policy on public.incrementality_tests
...
DROP POLICY IF EXISTS "authenticated_read" ON public.incrementality_tests;
DROP POLICY IF EXISTS "authenticated_read_incrementality_tests" ON public.incrementality_tests;

-- 3. Re-assert service_role bypass (Mission Control + n8n + backend admin)
DROP POLICY IF EXISTS "service_role_all" ON public.incrementality_tests;
CREATE POLICY "service_role_all" ON public.incrementality_tests
  FOR ALL
  TO service_role
  USING (true)
  WITH CHECK (true);
...
DROP POLICY IF EXISTS "client_isolation_incrementality_tests_select" ON public.incrementality_tests;
CREATE POLICY "client_isolation_incrementality_tests_select" ON public.incrementality_tests
  FOR SELECT
  TO authenticated
  USING (
...
DROP POLICY IF EXISTS "client_isolation_incrementality_tests_insert" ON public.incrementality_tests;
CREATE POLICY "client_isolation_incrementality_tests_insert" ON public.incrementality_tests
  FOR INSERT
  TO authenticated
  WITH CHECK (
    client_id = (auth.jwt() ->> 'client_id')::uuid
...
DROP POLICY IF EXISTS "client_isolation_incrementality_tests_update" ON public.incrementality_tests;
CREATE POLICY "client_isolation_incrementality_tests_update" ON public.incrementality_tests
  FOR UPDATE
  TO authenticated
  USING (
    client_id = (auth.jwt() ->> 'client_id')::uuid
...
DROP POLICY IF EXISTS "client_isolation_incrementality_tests_delete" ON public.incrementality_tests;
CREATE POLICY "client_isolation_incrementality_tests_delete" ON public.incrementality_tests
  FOR DELETE
  TO authenticated
  USING (
    client_id = (auth.jwt() ->> 'client_id')::uuid
...
COMMENT ON TABLE public.incrementality_tests IS
...
-- DROP POLICY IF EXISTS "client_isolation_incrementality_tests_update" ON public.incrementality_tests;
-- DROP POLICY IF EXISTS "client_isolation_incrementality_tests_delete" ON public.incrementality_tests;
-- CREATE POLICY "authenticated_read" ON public.incrementality_tests
