-- ============================================================================
-- Migration: 202604300001_rls_multi_tenant_social_schedules
-- Purpose: Replace permissive 'authenticated_read' RLS policy on public.social_schedules
...
DROP POLICY IF EXISTS "authenticated_read" ON public.social_schedules;
DROP POLICY IF EXISTS "authenticated_read_social_schedules" ON public.social_schedules;

-- 3. Re-assert service_role bypass (Mission Control + n8n + backend admin)
DROP POLICY IF EXISTS "service_role_all" ON public.social_schedules;
CREATE POLICY "service_role_all" ON public.social_schedules
  FOR ALL
  TO service_role
  USING (true)
  WITH CHECK (true);
...
DROP POLICY IF EXISTS "client_isolation_social_schedules_select" ON public.social_schedules;
CREATE POLICY "client_isolation_social_schedules_select" ON public.social_schedules
  FOR SELECT
  TO authenticated
  USING (
...
DROP POLICY IF EXISTS "client_isolation_social_schedules_insert" ON public.social_schedules;
CREATE POLICY "client_isolation_social_schedules_insert" ON public.social_schedules
  FOR INSERT
  TO authenticated
  WITH CHECK (
    client_id = (auth.jwt() ->> 'client_id')::uuid
...
DROP POLICY IF EXISTS "client_isolation_social_schedules_update" ON public.social_schedules;
CREATE POLICY "client_isolation_social_schedules_update" ON public.social_schedules
  FOR UPDATE
  TO authenticated
  USING (
    client_id = (auth.jwt() ->> 'client_id')::uuid
...
DROP POLICY IF EXISTS "client_isolation_social_schedules_delete" ON public.social_schedules;
CREATE POLICY "client_isolation_social_schedules_delete" ON public.social_schedules
  FOR DELETE
  TO authenticated
  USING (
    client_id = (auth.jwt() ->> 'client_id')::uuid
...
COMMENT ON TABLE public.social_schedules IS
...
-- DROP POLICY IF EXISTS "client_isolation_social_schedules_update" ON public.social_schedules;
-- DROP POLICY IF EXISTS "client_isolation_social_schedules_delete" ON public.social_schedules;
-- CREATE POLICY "authenticated_read" ON public.social_schedules
