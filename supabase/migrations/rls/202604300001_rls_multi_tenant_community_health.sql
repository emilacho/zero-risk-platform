-- ============================================================================
-- Migration: 202604300001_rls_multi_tenant_community_health
-- Purpose: Replace permissive 'authenticated_read' RLS policy on public.community_health
...
ALTER TABLE public.community_health ENABLE ROW LEVEL SECURITY;

-- 2. Drop the legacy permissive policies (multiple legacy names possible)
DROP POLICY IF EXISTS "authenticated_read" ON public.community_health;
DROP POLICY IF EXISTS "authenticated_read_community_health" ON public.community_health;

-- 3. Re-assert service_role bypass (Mission Control + n8n + backend admin)
DROP POLICY IF EXISTS "service_role_all" ON public.community_health;
CREATE POLICY "service_role_all" ON public.community_health
  FOR ALL
  TO service_role
  USING (true)
  WITH CHECK (true);
...
DROP POLICY IF EXISTS "client_isolation_community_health_select" ON public.community_health;
CREATE POLICY "client_isolation_community_health_select" ON public.community_health
  FOR SELECT
  TO authenticated
  USING (
...
DROP POLICY IF EXISTS "client_isolation_community_health_insert" ON public.community_health;
CREATE POLICY "client_isolation_community_health_insert" ON public.community_health
  FOR INSERT
  TO authenticated
  WITH CHECK (
    client_id = (auth.jwt() ->> 'client_id')
...
DROP POLICY IF EXISTS "client_isolation_community_health_update" ON public.community_health;
CREATE POLICY "client_isolation_community_health_update" ON public.community_health
  FOR UPDATE
  TO authenticated
  USING (
    client_id = (auth.jwt() ->> 'client_id')
...
DROP POLICY IF EXISTS "client_isolation_community_health_delete" ON public.community_health;
CREATE POLICY "client_isolation_community_health_delete" ON public.community_health
  FOR DELETE
  TO authenticated
  USING (
    client_id = (auth.jwt() ->> 'client_id')
...
COMMENT ON TABLE public.community_health IS
...
-- DROP POLICY IF EXISTS "client_isolation_community_health_update" ON public.community_health;
-- DROP POLICY IF EXISTS "client_isolation_community_health_delete" ON public.community_health;
-- CREATE POLICY "authenticated_read" ON public.community_health
