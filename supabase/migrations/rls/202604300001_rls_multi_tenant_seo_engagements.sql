-- ============================================================================
-- Migration: 202604300001_rls_multi_tenant_seo_engagements
-- Purpose: Replace permissive 'authenticated_read' RLS policy on public.seo_engagements
...
DROP POLICY IF EXISTS "authenticated_read" ON public.seo_engagements;
DROP POLICY IF EXISTS "authenticated_read_seo_engagements" ON public.seo_engagements;

-- 3. Re-assert service_role bypass (Mission Control + n8n + backend admin)
DROP POLICY IF EXISTS "service_role_all" ON public.seo_engagements;
CREATE POLICY "service_role_all" ON public.seo_engagements
  FOR ALL
  TO service_role
  USING (true)
  WITH CHECK (true);
...
DROP POLICY IF EXISTS "client_isolation_seo_engagements_select" ON public.seo_engagements;
CREATE POLICY "client_isolation_seo_engagements_select" ON public.seo_engagements
  FOR SELECT
  TO authenticated
  USING (
...
DROP POLICY IF EXISTS "client_isolation_seo_engagements_insert" ON public.seo_engagements;
CREATE POLICY "client_isolation_seo_engagements_insert" ON public.seo_engagements
  FOR INSERT
  TO authenticated
  WITH CHECK (
    client_id = (auth.jwt() ->> 'client_id')::uuid
...
DROP POLICY IF EXISTS "client_isolation_seo_engagements_update" ON public.seo_engagements;
CREATE POLICY "client_isolation_seo_engagements_update" ON public.seo_engagements
  FOR UPDATE
  TO authenticated
  USING (
    client_id = (auth.jwt() ->> 'client_id')::uuid
...
DROP POLICY IF EXISTS "client_isolation_seo_engagements_delete" ON public.seo_engagements;
CREATE POLICY "client_isolation_seo_engagements_delete" ON public.seo_engagements
  FOR DELETE
  TO authenticated
  USING (
    client_id = (auth.jwt() ->> 'client_id')::uuid
...
COMMENT ON TABLE public.seo_engagements IS
...
-- DROP POLICY IF EXISTS "client_isolation_seo_engagements_update" ON public.seo_engagements;
-- DROP POLICY IF EXISTS "client_isolation_seo_engagements_delete" ON public.seo_engagements;
-- CREATE POLICY "authenticated_read" ON public.seo_engagements
