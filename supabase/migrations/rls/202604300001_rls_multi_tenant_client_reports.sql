-- ============================================================================
-- Migration: 202604300001_rls_multi_tenant_client_reports
-- Purpose: Replace permissive 'authenticated_read' RLS policy on public.client_reports
...
DROP POLICY IF EXISTS "authenticated_read" ON public.client_reports;
DROP POLICY IF EXISTS "authenticated_read_client_reports" ON public.client_reports;

-- 3. Re-assert service_role bypass (Mission Control + n8n + backend admin)
DROP POLICY IF EXISTS "service_role_all" ON public.client_reports;
CREATE POLICY "service_role_all" ON public.client_reports
  FOR ALL
  TO service_role
  USING (true)
  WITH CHECK (true);
...
DROP POLICY IF EXISTS "client_isolation_client_reports_select" ON public.client_reports;
CREATE POLICY "client_isolation_client_reports_select" ON public.client_reports
  FOR SELECT
  TO authenticated
  USING (
...
DROP POLICY IF EXISTS "client_isolation_client_reports_insert" ON public.client_reports;
CREATE POLICY "client_isolation_client_reports_insert" ON public.client_reports
  FOR INSERT
  TO authenticated
  WITH CHECK (
    client_id = (auth.jwt() ->> 'client_id')::uuid
...
DROP POLICY IF EXISTS "client_isolation_client_reports_update" ON public.client_reports;
CREATE POLICY "client_isolation_client_reports_update" ON public.client_reports
  FOR UPDATE
  TO authenticated
  USING (
    client_id = (auth.jwt() ->> 'client_id')::uuid
...
DROP POLICY IF EXISTS "client_isolation_client_reports_delete" ON public.client_reports;
CREATE POLICY "client_isolation_client_reports_delete" ON public.client_reports
  FOR DELETE
  TO authenticated
  USING (
    client_id = (auth.jwt() ->> 'client_id')::uuid
...
-- DROP POLICY IF EXISTS "client_isolation_client_reports_update" ON public.client_reports;
-- DROP POLICY IF EXISTS "client_isolation_client_reports_delete" ON public.client_reports;
-- CREATE POLICY "authenticated_read" ON public.client_reports
