-- ============================================================================
-- Migration: 202604300001_rls_multi_tenant_experiments
-- Purpose: Replace permissive 'authenticated_read' RLS policy on public.experiments
...
DROP POLICY IF EXISTS "authenticated_read" ON public.experiments;
...
DROP POLICY IF EXISTS "service_role_all" ON public.experiments;
CREATE POLICY "service_role_all" ON public.experiments
  FOR ALL
  TO service_role
  USING (true)
  WITH CHECK (true);
...
DROP POLICY IF EXISTS "client_isolation_experiments_select" ON public.experiments;
CREATE POLICY "client_isolation_experiments_select" ON public.experiments
  FOR SELECT
  TO authenticated
  USING (
...
DROP POLICY IF EXISTS "client_isolation_experiments_insert" ON public.experiments;
CREATE POLICY "client_isolation_experiments_insert" ON public.experiments
  FOR INSERT
  TO authenticated
  WITH CHECK (
    client_id = (auth.jwt() ->> 'client_id')::uuid
...
DROP POLICY IF EXISTS "client_isolation_experiments_update" ON public.experiments;
CREATE POLICY "client_isolation_experiments_update" ON public.experiments
  FOR UPDATE
  TO authenticated
  USING (
    client_id = (auth.jwt() ->> 'client_id')::uuid
...
DROP POLICY IF EXISTS "client_isolation_experiments_delete" ON public.experiments;
CREATE POLICY "client_isolation_experiments_delete" ON public.experiments
  FOR DELETE
  TO authenticated
  USING (
    client_id = (auth.jwt() ->> 'client_id')::uuid
...
-- DROP POLICY IF EXISTS "client_isolation_experiments_update" ON public.experiments;
-- DROP POLICY IF EXISTS "client_isolation_experiments_delete" ON public.experiments;
-- CREATE POLICY "authenticated_read" ON public.experiments
