-- ============================================================================
-- Migration: 202604300001_rls_multi_tenant_rfm_segments
-- Purpose: Replace permissive 'authenticated_read' RLS policy on public.rfm_segments
...
DROP POLICY IF EXISTS "authenticated_read" ON public.rfm_segments;
DROP POLICY IF EXISTS "authenticated_read_rfm_segments" ON public.rfm_segments;

-- 3. Re-assert service_role bypass (Mission Control + n8n + backend admin)
DROP POLICY IF EXISTS "service_role_all" ON public.rfm_segments;
CREATE POLICY "service_role_all" ON public.rfm_segments
  FOR ALL
  TO service_role
  USING (true)
  WITH CHECK (true);
...
DROP POLICY IF EXISTS "client_isolation_rfm_segments_select" ON public.rfm_segments;
CREATE POLICY "client_isolation_rfm_segments_select" ON public.rfm_segments
  FOR SELECT
  TO authenticated
  USING (
...
DROP POLICY IF EXISTS "client_isolation_rfm_segments_insert" ON public.rfm_segments;
CREATE POLICY "client_isolation_rfm_segments_insert" ON public.rfm_segments
  FOR INSERT
  TO authenticated
  WITH CHECK (
    client_id = (auth.jwt() ->> 'client_id')
...
DROP POLICY IF EXISTS "client_isolation_rfm_segments_update" ON public.rfm_segments;
CREATE POLICY "client_isolation_rfm_segments_update" ON public.rfm_segments
  FOR UPDATE
  TO authenticated
  USING (
    client_id = (auth.jwt() ->> 'client_id')
...
DROP POLICY IF EXISTS "client_isolation_rfm_segments_delete" ON public.rfm_segments;
CREATE POLICY "client_isolation_rfm_segments_delete" ON public.rfm_segments
  FOR DELETE
  TO authenticated
  USING (
    client_id = (auth.jwt() ->> 'client_id')
...
COMMENT ON TABLE public.rfm_segments IS
...
-- DROP POLICY IF EXISTS "client_isolation_rfm_segments_update" ON public.rfm_segments;
-- DROP POLICY IF EXISTS "client_isolation_rfm_segments_delete" ON public.rfm_segments;
-- CREATE POLICY "authenticated_read" ON public.rfm_segments
