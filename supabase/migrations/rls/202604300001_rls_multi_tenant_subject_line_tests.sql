-- ============================================================================
-- Migration: 202604300001_rls_multi_tenant_subject_line_tests
-- Purpose: Replace permissive 'authenticated_read' RLS policy on public.subject_line_tests
...
ALTER TABLE public.subject_line_tests ENABLE ROW LEVEL SECURITY;

-- 2. Drop the legacy permissive policies (multiple legacy names possible)
DROP POLICY IF EXISTS "authenticated_read" ON public.subject_line_tests;
DROP POLICY IF EXISTS "authenticated_read_subject_line_tests" ON public.subject_line_tests;

-- 3. Re-assert service_role bypass (Mission Control + n8n + backend admin)
DROP POLICY IF EXISTS "service_role_all" ON public.subject_line_tests;
CREATE POLICY "service_role_all" ON public.subject_line_tests
  FOR ALL
  TO service_role
  USING (true)
  WITH CHECK (true);
...
DROP POLICY IF EXISTS "client_isolation_subject_line_tests_select" ON public.subject_line_tests;
CREATE POLICY "client_isolation_subject_line_tests_select" ON public.subject_line_tests
  FOR SELECT
  TO authenticated
  USING (
...
DROP POLICY IF EXISTS "client_isolation_subject_line_tests_insert" ON public.subject_line_tests;
CREATE POLICY "client_isolation_subject_line_tests_insert" ON public.subject_line_tests
  FOR INSERT
  TO authenticated
  WITH CHECK (
    client_id = (auth.jwt() ->> 'client_id')
...
DROP POLICY IF EXISTS "client_isolation_subject_line_tests_update" ON public.subject_line_tests;
CREATE POLICY "client_isolation_subject_line_tests_update" ON public.subject_line_tests
  FOR UPDATE
  TO authenticated
  USING (
    client_id = (auth.jwt() ->> 'client_id')
...
DROP POLICY IF EXISTS "client_isolation_subject_line_tests_delete" ON public.subject_line_tests;
CREATE POLICY "client_isolation_subject_line_tests_delete" ON public.subject_line_tests
  FOR DELETE
  TO authenticated
  USING (
    client_id = (auth.jwt() ->> 'client_id')
...
-- DROP POLICY IF EXISTS "client_isolation_subject_line_tests_update" ON public.subject_line_tests;
-- DROP POLICY IF EXISTS "client_isolation_subject_line_tests_delete" ON public.subject_line_tests;
-- CREATE POLICY "authenticated_read" ON public.subject_line_tests
