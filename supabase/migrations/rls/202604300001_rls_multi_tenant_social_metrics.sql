-- ============================================================================
-- Migration: 202604300001_rls_multi_tenant_social_metrics
-- Purpose: Replace permissive 'authenticated_read' RLS policy on public.social_metrics
...
DROP POLICY IF EXISTS "authenticated_read" ON public.social_metrics;
DROP POLICY IF EXISTS "authenticated_read_social_metrics" ON public.social_metrics;

-- 3. Re-assert service_role bypass (Mission Control + n8n + backend admin)
DROP POLICY IF EXISTS "service_role_all" ON public.social_metrics;
CREATE POLICY "service_role_all" ON public.social_metrics
  FOR ALL
  TO service_role
  USING (true)
  WITH CHECK (true);
...
DROP POLICY IF EXISTS "client_isolation_social_metrics_select" ON public.social_metrics;
CREATE POLICY "client_isolation_social_metrics_select" ON public.social_metrics
  FOR SELECT
  TO authenticated
  USING (
...
DROP POLICY IF EXISTS "client_isolation_social_metrics_insert" ON public.social_metrics;
CREATE POLICY "client_isolation_social_metrics_insert" ON public.social_metrics
  FOR INSERT
  TO authenticated
  WITH CHECK (
    client_id = (auth.jwt() ->> 'client_id')::uuid
...
DROP POLICY IF EXISTS "client_isolation_social_metrics_update" ON public.social_metrics;
CREATE POLICY "client_isolation_social_metrics_update" ON public.social_metrics
  FOR UPDATE
  TO authenticated
  USING (
    client_id = (auth.jwt() ->> 'client_id')::uuid
...
DROP POLICY IF EXISTS "client_isolation_social_metrics_delete" ON public.social_metrics;
CREATE POLICY "client_isolation_social_metrics_delete" ON public.social_metrics
  FOR DELETE
  TO authenticated
  USING (
    client_id = (auth.jwt() ->> 'client_id')::uuid
...
-- DROP POLICY IF EXISTS "client_isolation_social_metrics_update" ON public.social_metrics;
-- DROP POLICY IF EXISTS "client_isolation_social_metrics_delete" ON public.social_metrics;
-- CREATE POLICY "authenticated_read" ON public.social_metrics
